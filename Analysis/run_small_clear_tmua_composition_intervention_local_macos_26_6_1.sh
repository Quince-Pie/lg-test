...
# Execute the frozen small-clear Tmua intervention on the Retina M1.
...
readonly preregistration=Analysis/small_clear_tmua_composition_intervention_preregistration.json
readonly validator=Analysis/validate_small_clear_tmua_composition_intervention.py
readonly validator_test=Analysis/test_validate_small_clear_tmua_composition_intervention.py
readonly preflight=Analysis/check_local_retina_capture_session_v2.swift
readonly probe=Sources/GlassIntrospect/main.swift
readonly output_directory="local-small-clear-tmua-composition-${run_label}"
...
require_sha256 "$probe" d3f649b0d8985a5c4e5976319b2da55224429eaeab4002dd762a10957717d026
require_sha256 "$validator" c79aa9708813c224058316999a7835780aaf39f765a075a713003507a33d02c8
require_sha256 "$validator_test" 839d215d0a3c6add8d6bc056df59b67230f40d6b4002c604c02e4ad33b822a7d
require_sha256 "$preflight" f12a1cbe29629dc843cc3250a46fa686225f3c08bcf1bf1dbdf50aea913926f1
require_sha256 "$preregistration" c1a14074ea3714d8882addd560b6f730966ad7dd6abaf899c27f866ee1600719
...
readonly build_directory=$(/usr/bin/mktemp -d -t lg-small-tmua)
if [[ ! "$build_directory" =~ ^/var/folders/.+/T/lg-small-tmua\..+$ ]]; then
...
capture_status=0
while IFS='|' read -r case_id appearance direction geometry; do
    [[ -n "$case_id" ]] || continue
    ...
    capture_environment=(
        ...
        LG_TRANSITION_SMALL_CLEAR_TMUA_COMPOSITION_TRACE=1
    ...
    if [[ "$case_status" -ne 0 ]]; then
        capture_status=$case_status
        break
    fi
done <<'CASES'
clear-light-materialize-01|light|materialize|circle-combined-holdout-01
clear-dark-dematerialize-06|dark|dematerialize|circle-combined-holdout-06
CASES
