...
class Measurements:
    ...
    def clear_fixed_block_inventory(self) -> JsonObject:
        records: JsonObject = {}
        for background, metadata in CLEAR_FIXED_BLOCK_SWEEP_PROBES.items():
            ...
            records[background] = {
                "probeKind": metadata["probeKind"],
                "role": metadata["role"],
                "blockSizePixels": metadata["blockSizePixels"],
                "amplitudeCodes": metadata["amplitudeCodes"],
                "latticeSpacingPixels": metadata["latticeSpacingPixels"],
                "latticeGapPixels": metadata["latticeGapPixels"],
                "reducedBlockSizeCells": metadata["reducedBlockSizeCells"],
                "reducedGridSpacingCells": metadata[
                    "reducedGridSpacingCells"
                ],
                "referenceAvailable": background in self.references,
                "sourceControlRequired": bool(metadata["sourceControl"]),
                "sourceControlAvailable": control_available,
        ...
        return {
            ...
            "purpose": (
                "identify cumulative and sub-threshold clear-filter mass "
                "across fixed-site square size and amplitude"
            ),
            "catalogReferenceCount": len(CLEAR_FIXED_BLOCK_SWEEP_PROBES),
            "requiredSourceControlCount": sum(
                bool(metadata["sourceControl"])
    ...
    def run(self) -> JsonObject:
        ...
        if rig_version in {"2.17.0", "2.18.0", "2.19.0"}:
            result["clearFilterStageInventory"] = (
                self.clear_filter_stage_inventory()
            )
        if rig_version in {"2.18.0", "2.19.0"}:
            result["clearFixedImpulseInventory"] = (
                self.clear_fixed_impulse_inventory()
            )
        if rig_version == "2.19.0":
            result["clearFixedBlockInventory"] = (
                self.clear_fixed_block_inventory()
            )
        return result
