#!/usr/bin/env python3
"""Open schema-5 sealed records against the committed v3 prediction bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        ...
        raise ValueError("schema-5 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen prediction stream differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen prediction stream has trailing bytes")
...
def open_holdout(root: Path) -> JsonObject:
    ...
    predicted = {
        capture_case.name: (
            ...
            else model.case_stream(capture_case, selector_table)
        )
    ...
    return {
        "rasterTileTranslationHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-5 holdout differs")
