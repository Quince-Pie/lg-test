...
extern void lg_outer_refraction_intervention_sdf_name(void);
extern int32_t lg_outer_refraction_intervention_dump(
    const void *filter,
    uint64_t case_index);
...
int main(void)
{
    ...
    witness[1] = signed_witness_function(
        (const void *)&lg_outer_refraction_intervention_sdf_name,
        &witness[1]);
    ...
    while (fgets(line, (int)sizeof(line), stdin) != nullptr) {
        ...
        if (ordinal >= intervention_case_count ||
            !decode_parameters(line, parameters)) {
        ...
        const uint64_t case_index = (uint64_t)ordinal + 1;
        printf("CASE case_index=%llu object=", (unsigned long long)case_index);
        print_hex(background_filter, sizeof(background_filter));
        putchar('\n');
        if (lg_outer_refraction_intervention_dump(filter, case_index) != 0) {
            fputs("intervention serialization failed\n", stderr);
            return EXIT_FAILURE;
        }
        ++ordinal;
    }
    if (ferror(stdin) || ordinal != intervention_case_count) {
