#!/usr/bin/env python3
"""Materialize the schema-15 prediction archive and preregistration."""
...
def contract() -> dict[str, object]:
    ...
    return {
        "schemaVersion": 1,
        "role": capture.ROLE,
        "createdAt": "2026-08-01T22:17:00Z",
        "appleOutputsObservedAtPreregistration": False,
        "scientificQuestion": (
            "Does the AGX tile multiplier propagate carry through exactly the "
            "highest discarded product column, while discarding carry from "
            "all lower columns, and predict every pull, center, and derivative "
            "word for wholly novel geometry and endpoint bits?"
        ),
        "derivationEvidence": {
            "openedSchemas": list(range(3, 15)),
            "recordCount": 5_020_120,
            "wordCount": 90_362_160,
            "wordMismatchCount": 0,
            "retrospectiveExactAfterSchema14Calibration": True,
            "prospectiveAtDerivation": False,
            "schema14WasFailedProspectiveEvidence": True,
            "schema14RawSha256": (
                "77ff17088f516b5f736c78c9fbef108af00b4d0ea29f5ff8b3a30a251761d9d7"
            ),
            "schema14StickyPredictionWordMismatchCount": 1_440,
            "schema14CalibratedPredictionWordMismatchCount": 0,
        ...
        "model": {
            **model_source_hashes(),
            ...
            "tileProduct": (
                "individually low-19-bit-truncated binary partial products "
                "plus carry propagated through only discarded column 18, "
                "then bias 10 and 27-bit normalization"
            ),
        },
        ...
        "nextGate": (
            "Require zero mismatched words on this first unseen schema-15 "
