#!/usr/bin/env python3
"""Integrity checks for the second local provider field-matrix retry."""
...
PATH = (
    ANALYSIS
    / "case22_provider_field_matrix_local_macos_26_6_1_retry2_preregistration.json"
)


class Case22ProviderFieldMatrixRetry2PreregistrationTests(unittest.TestCase):
    @classmethod
    ...
    def test_failed_attempt_is_exact_and_has_no_authority(self) -> None:
        ...
        self.assertEqual(failed["statusBeforeFinalization"], "marker-armed")
        self.assertEqual(failed["intervalCount"], 0)
        self.assertEqual(failed["providerCallCount"], 0)
        self.assertEqual(failed["eventCount"], 0)
        self.assertEqual(failed["failureCount"], 0)
        self.assertFalse(failed["applicationReportPresent"])
    ...
    def test_retry_scope_cannot_change_the_matrix(self) -> None:
        ...
        self.assertTrue(retry["localUUIDOverlayImportedUnchanged"])
        for key, value in retry.items():
            if key.endswith("Changed"):
                self.assertFalse(value, key)
        self.assertIn("directly imported module", retry["onlyBehavioralChange"])
