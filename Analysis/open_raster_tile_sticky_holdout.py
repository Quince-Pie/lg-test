#!/usr/bin/env python3
"""Open schema-14 Apple records against the frozen sticky-carry bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        ...
        raise ValueError("schema-14 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen schema-14 prediction differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen schema-14 prediction has trailing bytes")
...
def open_holdout(root: Path) -> JsonObject:
    ...
    return {
        "rasterTileStickyHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
        ...
        * capture.RECORD_COMPONENT_COUNT,
        ...
        "remainingGates": [
            "unchanged independent schema-14 repeat",
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-14 sticky-carry holdout differs")
