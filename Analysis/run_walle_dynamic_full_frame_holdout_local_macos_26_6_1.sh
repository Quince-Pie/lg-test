...
# Run the frozen prospective eight-state Walle full-frame capture on the
# physical Retina Mac. Native build and capture deliberately exclude Nix.
...
readonly preregistration=Analysis/walle_dynamic_full_frame_holdout_preregistration.json
readonly preflight=Analysis/check_local_retina_capture_session_v2.swift
readonly probe=Sources/GlassIntrospect/main.swift
readonly output_root="local-walle-dynamic-full-frame-${run_label}-v1"
...
require_sha256 "$preregistration" 28592672c0ce68859ec9388218cfdcc22853257cd4d5fba67eaa87f08cd4aa4a
require_sha256 "$probe" 1f3f345cdba6bb328f6987a1948bac126ec9abca5356715272fc4eb3adea9e7e
...
readonly build_directory=$(/usr/bin/mktemp -d -t lg-walle-full-frame)
if [[ ! "$build_directory" =~ ^/var/folders/.+/T/lg-walle-full-frame\..+$ ]]; then
