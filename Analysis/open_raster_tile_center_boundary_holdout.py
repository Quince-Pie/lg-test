#!/usr/bin/env python3
"""Open schema-10 records against the committed v8 prediction bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        ...
        raise ValueError("schema-10 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen schema-10 prediction stream differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen schema-10 prediction stream has trailing bytes")
...
def open_holdout(root: Path) -> JsonObject:
    ...
    sealed_actual_sha256 = hashlib.sha256(combined_actual).hexdigest()
    exact = record_mismatches == 0
    return {
        "rasterTileCenterBoundaryHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-10 holdout differs")
