...
# Re-run one already-opened profile through the live QuartzCore transport.
...
readonly validator=Analysis/validate_prepare_layer_filter_map_bounds_profile_transfer_live_local_macos_26_6_1.py
readonly preflight=Analysis/check_local_retina_capture_session_v2.swift
readonly output_directory="local-prepare-layer-live-transport-calibration-${run_label}"
...

{
    git rev-parse HEAD
    /usr/bin/sw_vers
    /usr/bin/shasum -a 256 "$binary" "$capture" "$transport" "$validator" "$preflight"
