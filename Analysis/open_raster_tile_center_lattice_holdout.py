#!/usr/bin/env python3
"""Open schema-8 records against the committed v6 prediction bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        ...
        or frozen_model.get("v5SourceSha256") != sha256_path(Path(v5.__file__))
        ...
        raise ValueError("schema-8 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen schema-8 prediction stream differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen schema-8 prediction stream has trailing bytes")
...
def open_holdout(root: Path) -> JsonObject:
    ...
    return {
        "rasterTileCenterLatticeHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-8 holdout differs")
