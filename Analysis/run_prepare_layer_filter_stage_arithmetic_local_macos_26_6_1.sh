...
# Capture exact FilterOp and Glass DOD stages for the opened circle-498 case.
...
readonly capture=Analysis/capture_prepare_layer_filter_stage_arithmetic_local_macos_26_6_1_lldb.py
readonly preflight=Analysis/check_local_retina_capture_session_v2.swift
readonly output_directory="local-prepare-layer-filter-stage-arithmetic-${run_label}"
...

{
    git rev-parse HEAD
    /usr/bin/sw_vers
    /usr/bin/shasum -a 256 "$binary" "$capture" "$preflight"
...
/usr/bin/env "${capture_environment[@]}" \
    ...
    -o continue \
    -o "script import capture_prepare_layer_filter_stage_arithmetic_local_macos_26_6_1_lldb as stage_capture; stage_capture.finalize()" \
    -o quit \
