#!/usr/bin/env python3
"""Open schema-6 records against the committed v4 prediction bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        ...
        raise ValueError("schema-6 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen schema-6 prediction stream differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen schema-6 prediction stream has trailing bytes")
...
def open_holdout(root: Path) -> JsonObject:
    ...
    sealed_exact = mismatches_by_role["sealed-holdout"] == 0
    exact = record_mismatches == 0 and word_mismatches == 0
    return {
        "rasterTileDoubleRoundingHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
        ...
        "recordMismatchCount": record_mismatches,
        "wordMismatchCount": word_mismatches,
        "sealedExact": sealed_exact,
        "cases": cases,
        "mismatchExamples": examples,
        "exact": exact,
        "productionShaderAuthorized": False,
        "remainingGates": [
            "unchanged schema-6 repeat",
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-6 holdout differs")
