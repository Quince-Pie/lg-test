#!/usr/bin/env python3
"""Open schema-9 records against the committed v7 prediction bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        preregistration.get("sealedHoldoutOpenedAtPreregistration") is not False
        or frozen_model.get("sourceSha256") != sha256_path(Path(model.__file__))
        or frozen_model.get("baseSourceSha256") != sha256_path(Path(v1.__file__))
        or frozen_model.get("v2SourceSha256") != sha256_path(Path(v2.__file__))
        or frozen_model.get("v4SourceSha256") != sha256_path(Path(v4.__file__))
        or frozen_model.get("v6SourceSha256") != sha256_path(Path(v6.__file__))
        ...
        raise ValueError("schema-9 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen schema-9 prediction stream differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen schema-9 prediction stream has trailing bytes")
                    ...
                    examples.append(
                        {
                            ...
                            "cancellationDepth": model.cancellation_depth(endpoint),
...
def open_holdout(root: Path) -> JsonObject:
    preregistration, metadata, sealed_predictions = load_frozen_contract()
    validation, actual = actual_case_streams(root)
    selector_table = v1.load_selector_table()
    predicted = {
        capture_case.name: (
            sealed_predictions[capture_case.name]
            if capture_case.role == "sealed-holdout"
            else model.case_stream(capture, capture_case, selector_table)
        )
    ...
    for case in cases:
        role = str(case["role"])
        records_by_role[role] += int(case["recordCount"])
        words_by_role[role] += int(case["recordCount"]) * capture.RECORD_COMPONENT_COUNT
        mismatches_by_role[role] += int(case["wordMismatchCount"])
    sealed_actual = b"".join(
        actual[capture_case.name]
        for capture_case in capture.CASES
        if capture_case.role == "sealed-holdout"
    )
    sealed_actual_sha256 = hashlib.sha256(sealed_actual).hexdigest()
    exact = record_mismatches == 0 and word_mismatches == 0
    return {
        "rasterTileCenterScaleHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
        ...
        "sealedActualSha256": sealed_actual_sha256,
        "sealedPredictionHashExact": (
            sealed_actual_sha256 == preregistration["predictedTruthStream"]["sha256"]
        ),
        "recordCount": sum(int(case["recordCount"]) for case in cases),
        "wordCount": sum(words_by_role.values()),
        "recordsByCaseRole": dict(sorted(records_by_role.items())),
        "wordsByCaseRole": dict(sorted(words_by_role.items())),
        "wordMismatchesByCaseRole": dict(sorted(mismatches_by_role.items())),
        "recordMismatchCount": record_mismatches,
        "wordMismatchCount": word_mismatches,
        "sealedExact": mismatches_by_role["sealed-holdout"] == 0,
        "cases": cases,
        "mismatchExamples": examples,
        "exact": exact,
        "productionShaderAuthorized": False,
        "remainingGates": [
            "unchanged schema-9 repeat",
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-9 holdout differs")
