...
# Run one frozen materialize uniform-transfer case on the direct Retina Mac.
...
case "$material:$appearance:$geometry" in
    clear:light:circle-454-center)
        readonly case_name=clear-light-circle454
        ;;
    clear:dark:circle-462-center)
        readonly case_name=clear-dark-circle462
        ;;
    regular:light:circle-470-center)
        readonly case_name=regular-light-circle470
        ;;
    regular:dark:circle-478-center)
        readonly case_name=regular-dark-circle478
        ;;
    *)
        echo "profile is not one frozen materialize uniform holdout case" >&2
        exit 64
        ;;
esac
...
readonly native_clamp=Analysis/analyze_transition_uniform_profile_clamp_local_macos_26_6_1.swift
readonly calibration=Analysis/transition_uniform_profile_calibration_result.json
readonly validator=Analysis/validate_transition_uniform_profile_holdout.py
readonly preregistration=Analysis/transition_uniform_profile_holdout_preregistration.json
readonly preflight=Analysis/check_local_retina_capture_session_v2.swift
readonly output_directory="local-transition-uniform-profile-${run_label}"
...
require_sha256 "$native_clamp" d1b3a8a2e95c54bcadea836560c9091f211cb2cf8ad2efc03d43e15040da01af
require_sha256 "$calibration" 3f3c9e657378e528653bd8c255fe7ac8bb632331a0bfe8a6a39ab870045a77d5
require_sha256 "$validator" 90ff6ccb954c08d958d24b0d17a7611ad42ab26c92c538b622ea9e67c796fb98
require_sha256 "$preregistration" 29672bc86b220e21ff513712eb2cfa15f5db650a5c2db11b15caee525f999a71
...

{
    ...
    /usr/bin/shasum -a 256 \
        "$binary" "$model" "$native_clamp" "$calibration" \
