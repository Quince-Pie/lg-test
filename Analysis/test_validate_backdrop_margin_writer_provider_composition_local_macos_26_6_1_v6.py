"""Unit tests for the corrected public/provider/Group/writer composition law."""
...
import validate_backdrop_margin_writer_provider_composition_local_macos_26_6_1_v6 as validator
...
class BackdropMarginWriterProviderCompositionTests(unittest.TestCase):
    ...
    def test_regular_candidate_uses_provider_and_group_contribution(self) -> None:
        ...
        self.assertEqual(
            candidate["perRecordRegularGroupContributionLaw"],
            validator.REGULAR_CONTRIBUTION_LAW,
        )
        self.assertEqual(candidate["providerMaximumF64"], 83.0)
        self.assertEqual(candidate["bleedMaximumF64"], 40.0)
        self.assertEqual(candidate["regularGroupContributionMaximumF64"], 83.0)
        ...
        self.assertTrue(candidate["candidateLawCalibratedFromOpenedWriterTarget"])
        self.assertFalse(
            candidate["capturedWriterValueReadDuringCandidateEvaluation"]
        )
        self.assertFalse(candidate["prospectiveFreshCaseOutputUsedToBuildCandidate"])
    ...
    def test_axis_max_and_absolute_shape_are_not_the_old_fitted_law(self) -> None:
        ...
        self.assertEqual(
            candidate["records"][-1]["regularGroupContributionF64"], 10_000.0
        )
        self.assertEqual(candidate["providerMaximumF64"], 80.65625)
        self.assertEqual(candidate["bleedMaximumF64"], 10_000.0)
        self.assertEqual(candidate["regularGroupContributionMaximumF64"], 10_000.0)
        self.assertEqual(candidate["maximumRequiredMarginF64"], 10_000.0)
