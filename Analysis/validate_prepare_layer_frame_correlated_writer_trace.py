...
EXPECTED_CONFIGURATION = {
    "captureBackdropSymbol": full_base.CAPTURE_BACKDROP_SYMBOL,
    "captureBackdropCodeByteCount": full_base.CAPTURE_BACKDROP_CODE_BYTE_COUNT,
    "captureBackdropCodeSHA256": full_base.CAPTURE_BACKDROP_CODE_SHA256,
...
def _static_gates(
    ...
    if (
        ...
        or capture.get("codeSHA256") != full_base.CAPTURE_BACKDROP_CODE_SHA256
        or late_id <= 0
    ):
    ...
    for expected, value in zip(WRITER_SITES, values, strict=True):
        ...
        if (
            ...
            or not integer(symbol.get("startAddress"), f"{label} start")
            <= address
            < integer(symbol.get("endAddress"), f"{label} end")
        ):
            raise ValueError(f"{label} identity differs")
    ...
    if len(set(breakpoint_ids)) != len(breakpoint_ids) or breakpoint_ids != sorted(
        breakpoint_ids
    ):
    ...
    *,
    ...
    for index, value in enumerate(values):
        ...
        if (
            ...
            or not isinstance(event.get("sourceKnownAtHit"), bool)
            or thread_id <= 0
        ...
        previous_index = None if site["epochStart"] else last_by_frame.get(
            identity_key
        )
        ...
        changed = None if previous_aggregate is None else aggregate != previous_aggregate
        if (
        ...
        general = _top_operands(
            event.get("topOperandSnapshot"), label + " operands", expected_pc=stop_pc
        )
...
def _writer_accounting(
    ...
    *,
    ...
    if not all(set(values) == expected_names for values in (hit_counts, rejected_counts, discarded_counts)):
...
def _selection(
    ...
    *,
    ...
    hit = integer(selected.get("markerHitIndex"), "selected marker hit")
    ...
    registers = _registers(
        selected.get("registers"), PREPARE_FRAME_REGISTER_NAMES, "selected registers"
    )
    ...
    selected_indices = list(
        sequence(trace.get("selectedWriterEventIndices"), "selected event indices")
    )
    selected_indices = [
        integer(value, "selected event index") for value in selected_indices
    ]
    if (
    ...
    epoch_starts = [event for event in matching if event.get("epochStart") is True]
    if not epoch_starts:
        raise ValueError("selected writer epoch is absent")
    epoch = max(epoch_starts, key=lambda event: event["callbackSequence"])
    expected_selected = [
        event["eventIndex"]
