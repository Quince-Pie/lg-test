#!/usr/bin/env python3
"""Open schema-7 records against the committed v5 prediction bytes."""
...
def load_frozen_contract() -> tuple[JsonObject, JsonObject, dict[str, bytes]]:
    ...
    if (
        ...
        raise ValueError("schema-7 model or prediction contract differs")
    ...
    for case_metadata in metadata["cases"]:
        ...
        if hashlib.sha256(stream).hexdigest() != case_metadata["sha256"]:
            raise ValueError(f"frozen schema-7 prediction stream differs for {name}")
        streams[name] = stream
        offset += byte_count
    if offset != len(combined):
        raise ValueError("frozen schema-7 prediction stream has trailing bytes")
...
def open_holdout(root: Path) -> JsonObject:
    ...
    return {
        "rasterTileCenterOriginHoldoutOpeningSchemaVersion": 1,
        "source": str(root),
...
def main() -> None:
    ...
    if not report["exact"]:
        raise SystemExit("schema-7 holdout differs")
