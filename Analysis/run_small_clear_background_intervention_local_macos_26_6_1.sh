...
# Execute the frozen small-clear Tghn intervention on the Retina M1.
...
readonly preregistration=Analysis/small_clear_background_intervention_preregistration.json
readonly validator=Analysis/validate_small_clear_background_intervention.py
readonly validator_test=Analysis/test_validate_small_clear_background_intervention.py
readonly retained_result=Analysis/small_clear_background_result.json
readonly preflight=Analysis/check_local_retina_capture_session_v2.swift
readonly probe=Sources/GlassIntrospect/main.swift
readonly output_directory="local-small-clear-background-${run_label}"
...
require_sha256 "$probe" c2f74579b8b57017c1dd9260c4ce6dbc7ead8183370e2e1d6f8ed4c750e2e8b5
require_sha256 "$validator" fead760663f2f853c0994a8f1cf0ebef3f37f19f780ba915484e8973e01a0c66
require_sha256 "$validator_test" cce0a21b7531202051a3655c9cecb22bdfdfc106ea6d138a59f4f41ce19a5df5
require_sha256 "$retained_result" cc21d4fa6c67f614c83941b544a01a9f24da8e39f7600651561b710e180b8359
require_sha256 "$preflight" f12a1cbe29629dc843cc3250a46fa686225f3c08bcf1bf1dbdf50aea913926f1
require_sha256 "$preregistration" e649ad01476283caec2a1ec4884de2b5c41a9e3ca9060bf75f52a016f1da7581
...
readonly build_directory=$(/usr/bin/mktemp -d -t lg-small-background)
if [[ ! "$build_directory" =~ ^/var/folders/.+/T/lg-small-background\..+$ ]]; then
...
"$nix" --extra-experimental-features "nix-command flakes" \
    develop --command env PYTHONPATH=Analysis python -m unittest \
        Analysis/test_validate_small_clear_background_intervention.py \
...
readonly -a capture_environment=(
    ...
    LG_TRANSITION_SMALL_CLEAR_BACKGROUND_TRACE=1
...

{
    ...
    /usr/bin/shasum -a 256 \
        ...
        "$retained_result" "$preflight" "$preregistration" \
