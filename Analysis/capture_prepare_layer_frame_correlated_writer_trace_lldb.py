"""LLDB capture correlating early writers to a later live selected frame.

The preceding watchpoint run proved that ``x28`` is not yet the selected
source at early direct writes.  This probe records exact writer sites first,
then classifies their still-live ``prepare_layer`` invocation at +0x3ef0 by
...
DEFAULT_TRACE_OUTPUT = (
    "transition-introspection/prepare-layer-frame-writer-trace.json"
)
...
_state = {
    ...
    "rejectedWriterSiteHitCounts": {site["name"]: 0 for site in WRITER_SITES},
    "discardedWriterSiteHitCounts": {site["name"]: 0 for site in WRITER_SITES},
...
def _writer_site_configuration():
    return [dict(site) for site in WRITER_SITES]
...
def _new_trace():
    return {
        ...
        "configuration": {
            ...
            "writerSites": _writer_site_configuration(),
            "maximumWriterSiteHitCount": MAXIMUM_WRITER_SITE_HIT_COUNT,
            "maximumRecordCountPerWriterSite": (
                MAXIMUM_RECORD_COUNT_PER_WRITER_SITE
            ),
...
def _disable_internal_breakpoints():
    if _state["selectionMarkerBreakpoint"] is not None:
        _state["selectionMarkerBreakpoint"].SetEnabled(False)
...
def _top_operand_snapshot(frame):
    ...
    return {
        ...
        "registerPointerProbeFailures": pointer_failures,
    }
...
def _matching_prepare_frame(thread):
    ...
    for index in range(
        ...
        try:
            ...
            continue
        values = {record["name"]: record["unsignedValue"] for record in registers}
...
def capture_backdrop_late(frame, _breakpoint_location, _internal_dict):
    """Select the exact preconvergence source independently of writer sites."""
    try:
        process = frame.GetThread().GetProcess()
        ...
        _state["captureLateBreakpoint"].SetEnabled(False)
        _state["trace"]["status"] = "source-selected-awaiting-live-frame-correlation"
        _write_trace()
    except Exception as error:
...
def prepare_layer_entry(frame, breakpoint_location, _internal_dict):
    """Gate full code and install all writer and selection-marker sites."""
    try:
        ...
        for site in WRITER_SITES:
            previous_offset = site.get("precedingInstructionRelativeToPrepareLayer")
            expected_raw = site.get("precedingInstructionRawLittleEndianHex")
            if previous_offset is None:
                continue
            if previous_offset < 0:
                helper_offset = previous_offset - capture_base.UNION_HELPER_RELATIVE_TO_PREPARE_LAYER
                observed = helper_code[helper_offset : helper_offset + 4].hex()
            else:
                observed = code[previous_offset : previous_offset + 4].hex()
        ...
        for site in WRITER_SITES:
            ...
            preceding_instruction = capture_base._read_memory(
                process,
                address - 4,
                4,
                site["name"] + " preceding instruction",
            )
            ...
            writer_site_records.append(
                {
                    **site,
                    ...
                    "precedingInstructionRawLittleEndianHex": (
                        preceding_instruction.hex()
                    ),
                }
            )
        ...
        prepare = {
            ...
            "writerSites": writer_site_records,
            "liveSelectionMarker": {
...
def _record_rejected_writer(frame, site, reason):
    _state["rejectedWriterSiteHitCounts"][site["name"]] += 1
    target = frame.GetThread().GetProcess().GetTarget()
    key = (site["name"], frame.GetPC(), frame.GetFunctionName(), str(reason))
...
def writer_site(frame, breakpoint_location, _internal_dict):
    """Capture a bounded writer site and its nearest exact prepare frame."""
    try:
        address = breakpoint_location.GetAddress().GetLoadAddress(
        ...
        if prepare_frame is None:
            ...
            return False
        retained_for_site = sum(
            event["siteName"] == name
            for event in _state["trace"]["writerCandidateEvents"]
        )
        if retained_for_site >= MAXIMUM_RECORD_COUNT_PER_WRITER_SITE:
            _state["discardedWriterSiteHitCounts"][name] += 1
        ...
        if previous_index is not None:
            previous_aggregate = bytes.fromhex(
                _state["trace"]["writerCandidateEvents"][previous_index][
                    "aggregateAfterHex"
                ]
            )
        sequence = _next_sequence("writer-site:" + name)
        event = {
            "eventIndex": len(_state["trace"]["writerCandidateEvents"]),
            "callbackSequence": sequence,
            "siteName": name,
            ...
            "aggregateChangedFromPreviousSameFrameCandidate": (
                ...
                else aggregate != previous_aggregate
            ),
            "roleStateAfter": {
                "address": role_base,
                "byteCount": len(role),
        ...
        _state["trace"]["writerCandidateEvents"].append(event)
...
def _selected_object_snapshots(process):
    return {
        base_name: capture_base._memory_snapshot(
            process,
            _state["objectAddresses"][base_name],
            byte_count,
            "selected " + base_name + " at live marker",
        )
        for base_name, byte_count in capture_base.OBJECT_SNAPSHOT_SPECS
    }
...
def live_selection_marker(frame, _breakpoint_location, _internal_dict):
    ...
    try:
        ...
        if (
            _state["selectionMarkerHitCount"]
            > MAXIMUM_LIVE_SELECTION_MARKER_HIT_COUNT
        ):
        ...
        matching = [
            event
            for event in _state["trace"]["writerCandidateEvents"]
            if event["frameIdentity"] == frame_identity
        ]
        epoch_starts = [
            event for event in matching if event.get("epochStart") is True
        ]
        if not epoch_starts:
            raise RuntimeError("selected live frame has no writer epoch start")
        epoch_start = max(
            epoch_starts, key=lambda event: event["callbackSequence"]
        )
        selected = [
            event
            for event in matching
            if event["callbackSequence"] >= epoch_start["callbackSequence"]
        ]
        ...
        _state["trace"]["selectedFrame"] = {
            ...
            "epochStartEventIndex": epoch_start["eventIndex"],
...
def finalize():
    """Finalize exact writer-site, marker, and selected-suffix accounting."""
    trace = _state["trace"]
    if trace is None:
        return
    ...
    trace["finalWriterCandidateEventCount"] = len(
        trace["writerCandidateEvents"]
    )
    ...
    trace["finalSelectedWriterEventCount"] = len(
        trace["selectedWriterEventIndices"]
    )
    trace["finalSelectedDistinctAggregateCount"] = len(
        {
            trace["writerCandidateEvents"][index]["aggregateAfterHex"]
            for index in trace["selectedWriterEventIndices"]
        }
    )
